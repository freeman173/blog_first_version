...
如何在控制台调试vue：
在 mounted() 方法中将 this 的值赋值给 window.vue：
mounted() {
window.vue = this
}
...
1、data() 变为 setUp()：
    ...
    setup函数里面有两种定义变量的方法，分别是 ref 和 reactive:
        ref 用于定义基本数据类型，比如string、number、boolean等.
        reactive用于定义复杂数据类型，比如数组、对象等。
    setup位于created 和beforeCreated之前,用于代替created 和beforeCreated,
    执行 setup 时，组件实例尚未被创建（在 setup() 内部，this 不会是该活跃实例的引用，即不指向vue实例
2、
...
3、
const的作用就是将变量声明为一个常量，不能被修改，不可以再次赋值，它的指向是不可以改变的，只能指向一个固定区域。
