...
<!--        <myarticle-scroll-page v-bind="article"></myarticle-scroll-page>-->
        <MyArticleScrollPage v-bind="article"></MyArticleScrollPage>
...
<script>
  import MyArticleScrollPage from '@/views/common/MyArticleScrollPage'
  import {mylistArchives} from '@/api/article'
  ...
  export default {
    ...
    methods: {
      ...
      changeArchive(year, month) {
        ...
        this.$router.push({path: `/myarchives/${year}/${month}`})
      },

      listArchives() {
        let authorName=this.$store.state.name
        mylistArchives(authorName,this.$store.state.token).then((data => {
